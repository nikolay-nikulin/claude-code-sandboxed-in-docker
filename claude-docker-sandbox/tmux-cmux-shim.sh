...
build_host_exec_cmd() {
    ...
    if [ "$IS_SBX" = "1" ] || [ -n "$SANDBOX_NAME" ]; then
        ...
        if [ -n "$cmd_args" ]; then
            echo "exec env PATH=/usr/local/bin:/opt/homebrew/bin:/usr/bin:/bin docker exec -it ${CONTAINER_TARGET} ${cmd_args}"
        else
            echo "exec env PATH=/usr/local/bin:/opt/homebrew/bin:/usr/bin:/bin docker exec -it ${CONTAINER_TARGET} bash"
        fi
    fi
}
